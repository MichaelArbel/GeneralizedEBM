...
def main(d_types):
    ...
    for d in d_types:
        if on_sshfs:
            fname = os.path.join('logs', d, 'fids', 'lmc_fids.json')
        else:
            fname = os.path.join('sshfs', 'logs', d, 'fids', 'lmc_fids.json')
        print(fname)
        ...
        x_range = np.arange(0, 100, step=10)

        # plt.plot(x_range, fid_train, '.', linestyle='-', label=model_map[d], lw=2)

        plt.plot(x_range, fid_test, '.', linestyle='-.', label=model_map[d], lw=2)
