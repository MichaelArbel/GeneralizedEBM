...
def get_activations(images, model, device,batch_size=50, dims=2048, verbose=True):
    """Calculates the activations of the pool_3 layer for all images.
    Params:
    -- files       : List of image files paths
    -- model       : Instance of inception model
...
def get_activations_from_loader(dataloader, model, device,batch_size=50, dims=2048, verbose=True):
    ...
    for batch_idx, (data, target) in enumerate(dataloader):
        if verbose:
            print('\rPropagating batch %d/%d' % (batch_idx + 1, n_batches),
                  end='', flush=True)
...
def compute_stats_from_loader(model,data_loader,device,batch_size=128):
